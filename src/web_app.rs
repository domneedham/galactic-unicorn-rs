...
impl UnicornAppRunner for WebAppRunner {
    async fn run(&mut self) -> ! {
        // Set notification policy to deny normal notifications (like Draw app)
        self.notification_policy
            .signal(AppNotificationPolicy::DenyNormal);

        // Clear screen to start with blank canvas
                ...
                .await
                {
                    ...
                    Err(draw_protocol::ParseError::NeedMoreData) => {
                        ...
                        if acc_len > 2400 && !made_progress {
                            log::warn!("WebApp: Accumulator stalled at {} bytes, resetting", acc_len);
                            acc_len = 0;
                            consumed = 0;
                        }
                        break;
